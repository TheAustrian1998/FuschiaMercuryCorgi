...
task("bridge", "bridge...")
    ...
    .setAction(async (taskArgs) => {
        ...
        let lockNSendWithAmount = new ethers.Contract(taskArgs.side, [
            'function lockNSend(uint amount) external'
        ], this.deployer);

        let lockNSendWithReceiver = new ethers.Contract(taskArgs.side, [
            'function lockNSend(uint amount, address receiver) external'
        ], this.deployer);
        ...
        if (taskArgs.receiver != null && taskArgs.receiver != undefined && taskArgs.receiver) {
            // optional parm send
            tx = await lockNSendWithReceiver.lockNSend(amountToSend, receiver, {
                gasLimit: 1000000
            });
        } else {
            // parm not send
            tx = await lockNSendWithAmount.lockNSend(amountToSend, {
                gasLimit: 1000000
            });
        }
